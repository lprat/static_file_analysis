...
rule Advapi_Hash_API {
	...
	condition:
		check_crypt_bool and $advapi32 and ($CryptCreateHash and $CryptHashData and $CryptAcquireContext)
}
...
rule Crypt32_CryptBinaryToString_API {
	...
	condition:
		check_crypt_bool and $crypt32 and ($CryptBinaryToStringA)
}
...
rule Elf_Hash {
	...
	condition:
		check_crypt_bool and any of them
}
...
rule BLOWFISH_Constants {
	...
	strings:
		...
		$c11 = { 1C264CF6 }
	condition:
		check_crypt_bool and 6 of them
}
...
rule MD5_Constants {
	...
	strings:
		...
		$c9 = { 78A46AD7 }
	condition:
		check_crypt_bool and 5 of them
}
...
rule MD5_API {
	...
	condition:
		check_crypt_bool and ($advapi32 or $cryptdll) and ($MD5Init and $MD5Update and $MD5Final)
}
...
rule RC6_Constants {
	...
	strings:
		...
		$c4 = { B979379E }
	condition:
		check_crypt_bool and 2 of them
}
...
rule SHA2_BLAKE2_IVs {
	...
	condition:
		check_crypt_bool and all of them
}
...
rule DarkEYEv3_Cryptor {
	...
	strings:
		$s0 = "\\DarkEYEV3-" 
	condition:
		check_crypt_bool and uint16(0) == 0x5a4d and $s0
}
